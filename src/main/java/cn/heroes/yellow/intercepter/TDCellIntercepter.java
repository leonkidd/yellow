...
/**
 * 二维表单元格(横标尺为ABC..., 纵标尺为123...)取值, 获取指定的单元格的数值. 单元格标识代表单元格位置(e.g. H3). Usage:
 * 实现<code>callback</code>方法, 实例一个实现类对象,并传入要解析的单元格位置名称数组.
 * 
 * @author Leon Kidd
...
public abstract class TDCellIntercepter implements TDIntercepter {
	...
	public void init() {
		...
		for (String cellPos : cellPoses) {
			// 单元格位置名称转为单元格位置二维数组
			int[] p = CellUtils.name2pos(cellPos);
			// TODO exception
			int row = p[0];
			int col = p[1];
	...
	public void row(TDRow row) {
		...
		for (int i : list) {
			...
			// 该单元格的位置名称
			String key = CellUtils.pos2name(rowNum, i);
